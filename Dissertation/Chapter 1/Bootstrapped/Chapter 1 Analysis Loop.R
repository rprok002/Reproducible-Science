...
for( a in unique(data$Frog.Number)){
  for( b in unique(data$Trial.Type)){
    ...
    for( c in unique(data$Trial.Number)){
      ...
      ## Random sample
      F1T1N1s <- F1T1N1[sample(nrow(F1T1N1), 720, replace = TRUE),]
      ...
      Control <- c(sum(F1T1N1c$Number.of.Seconds)/720)
      Experiment <- c(sum(F1T1N1e$Number.of.Seconds)/720)
...
marginF1T1N1c <- qnorm(0.975)*sqrt(mean(as.numeric(newF1T1N1c$Seconds))*(1-mean(as.numeric(newF1T1N1c$Seconds)))/20)
